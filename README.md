...
| Слой | Технология |
|------|-----------|
| Desktop | Electron 42, contextIsolation, sandbox=false |
...
PORT=3000
CORS_ORIGIN=null,http://localhost:5173
...
### 2026-05-18 — Codex — security hardening pass

**Request:** использовать security-проверку и сделать все нужные исправления.
...
- `studingJS/src/runtime/childProcessSafety.js`
- `studingJS/src/runtime/executor.js`
- `studingJS/src/runtime/testRunner.js`
...
**What changed:**
- Electron поднят до `42.1.0`; frontend `npm audit` теперь без high vulnerabilities.
- Server dev audit закрыт через scoped `esbuild` override для `@esbuild-kit/core-utils`; production и full audit дают 0 vulnerabilities.
- CORS больше не советует и не принимает `*` в production: default стал `null,http://localhost:5173`, а `credentials` выключается для wildcard.
- Stripe webhook теперь требует `STRIPE_WEBHOOK_SECRET` в production и при любом включенном Stripe secret; неподписанный JSON fallback оставлен только для полностью локального dev/test без Stripe.
- Refresh token rotation стал атомарным: старый token поглощается через один `DELETE ... RETURNING`, поэтому параллельный reuse не получает второй новый refresh token.
- Gemini API key больше не передается в URL query, уходит через `x-goog-api-key` header.
- Native runners Python/Go/C/C++/Java/C# запускают пользовательский код с timeout и scrubbed environment без `DATABASE_URL`, JWT, Stripe/Gemini/Resend/Sentry/GitHub/OpenAI secrets.
- JS practice runner больше не передает host constructors/timers внутрь `vm` sandbox; закрыт `Function('return process')` escape через global/callback/DOM helpers.
- Electron main window запрещает new-window и навигацию вне локального renderer file URL.
- Go task metadata валидирует типы через allowlist (`int`, `string`, `bool`, slices/maps из них), чтобы `returnType/argTypes` не могли инжектить Go-код в runner.
- Python runner в `%TEMP%` каждый раз перезаписывается из bundled source и не доверяет старому файлу с тем же hash-name.
- Release checklist теперь запускает frontend/backend security contracts и не допускает high vulnerabilities; Windows installer пересобран и обновлен в GitHub Release `v1.0.0` (`JS.Infinite.Trainer.Setup.1.0.0.exe`, size `112950066`, sha256 `a5765646aac8dc35041e7036b3bc21f34ea52c9534f2f46f7c284a46dc5f2e76`).

**Verification:**
- `npm run test:security` в `server` — passed, 7/7.
- `npm run security:contract` в `studingJS` — passed.
- `npm run build` в `server` — passed.
- `npm audit --audit-level=moderate` в `server` — passed, 0 vulnerabilities.
- `npm audit --audit-level=moderate` в `studingJS` — passed, 0 vulnerabilities.
- `node --check ...` для измененных Electron/runtime/kernel файлов — passed.
- `npm run smoke`, `tasks:contract`, `runtime:timeout`, `python:variation`, `go:smoke`, `go:variation`, `java:variation`, `csharp:variation`, `theory:practice`, `theory:coverage`, `theory:content`, `theory:scroll`, `qa:trust`, `fallback`, `review:loop`, `report:serialization`, `diversity`, `weighting`, `drafts:persistence`, `variation` — passed.
- C/C++ manual kernel smoke — passed: solution passes, starter fails as expected.
- `node src/tests/kernelVariationGuard.js c` и `cpp` — failed on pre-existing `answerFormat` contract, unrelated to this security pass.
- `git diff --check` — passed.
- `node release.mjs` — passed: 12/12, installer rebuilt, production `/health` ok.
- `gh release upload v1.0.0 ... --clobber` — passed, release asset verified.

**Coordination notes:**
- Railway production must not use `CORS_ORIGIN=*`; for the Electron app set `CORS_ORIGIN=null`, and add web domains comma-separated if a browser client appears.
- If Stripe billing is enabled in production, `STRIPE_WEBHOOK_SECRET` is now mandatory.
- Native code is now time-bounded and env-scrubbed, but it is still local code execution, not a full OS/container sandbox.
...
**Changes:**
- Чекбоксы режимов (random, infinite, auto-hint) скрыты для беты через `display:none` — код сохранён, легко вернуть
- Anti-cheat: если открыл кнопку "Ответ" — XP не начисляется, серия сбрасывается, toast объясняет почему
- Кнопка "✨ ИИ" перенесена в hero-actions рядом с "Подсказка", ответ ИИ появляется над списком тестов
- Gemini Flash Lite подключён (GEMINI_API_KEY в Railway) — AI hints и AI breakdown работают

 Этот раздел нужен, чтобы пользователь, Codex и Claude Code видели, какие изменения сделал Codex и как они проверялись.
